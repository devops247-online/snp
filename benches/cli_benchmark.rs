...
fn benchmark_cli_parsing(c: &mut Criterion) {
    ...
    c.bench_function("cli_parse_run_complex", |b| {
        b.iter(|| {
            let result = Cli::try_parse_from(black_box(&[
                "snp", "run", "--all-files", "--verbose", "--show-diff-on-failure",
                "--files", "file1.py", "file2.rs", "--hook-stage", "pre-push"
            ]));
            black_box(result);
        })
    });
    ...
    c.bench_function("cli_parse_install_complex", |b| {
        b.iter(|| {
            let result = Cli::try_parse_from(black_box(&[
                "snp", "install", "--hook-type", "pre-commit", "--hook-type", "pre-push",
                "--overwrite", "--install-hooks", "--allow-missing-config"
            ]));
            black_box(result);
        })
    });
    ...
    c.bench_function("cli_parse_autoupdate_complex", |b| {
        b.iter(|| {
            let result = Cli::try_parse_from(black_box(&[
                "snp", "autoupdate", "--bleeding-edge", "--freeze",
                "--repo", "repo1", "--repo", "repo2", "--jobs", "4"
            ]));
            black_box(result);
        })
    });
