...
pub enum WorkerState {
    Idle,
    Working { task_id: String, started_at: SystemTime },
    Stealing,
}
...
impl LockFreeTaskScheduler {
    ...
    pub fn submit_task<F>(&self, config: TaskConfig, task: F) -> Result<oneshot::Receiver<TaskResult<()>>>
    where
    ...
    pub fn submit_batch<F>(&self, tasks: Vec<(TaskConfig, F)>) -> Result<Vec<oneshot::Receiver<TaskResult<()>>>>
    where
        F: FnOnce() -> BoxFuture<'static, Result<()>> + Send + 'static,
    {
    ...
    pub fn mark_task_completed(&self, worker_id: usize, _task_id: &str, duration: Duration) {
        ...
        self.total_task_duration_ms.fetch_add(duration.as_millis() as u64, Ordering::Relaxed);
        self.worker_states.insert(worker_id, WorkerState::Idle);
    }
    ...
    pub fn get_stats(&self) -> SchedulerStats {
        ...
        let active_workers = self.worker_states
            .iter()
    ...
    pub fn get_worker_state(&self, worker_id: usize) -> Option<WorkerState> {
        self.worker_states.get(&worker_id).map(|entry| entry.value().clone())
    }
...
mod tests {
    ...
    async fn test_basic_task_submission() {
        ...
        let _receiver = scheduler.submit_task(config, || {
            Box::pin(async move {
                tokio::time::sleep(Duration::from_millis(10)).await;
                Ok(())
            })
    ...
    async fn test_work_stealing() {
        ...
        for i in 0..8 {
            ...
            let receiver = scheduler.submit_task(config, move || {
                Box::pin(async move {
