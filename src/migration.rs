...
use crate::error::{Result, SnpError, ConfigError};
...
pub enum ValidationType {
    ...
    ValidEnum { field_path: String, allowed_values: Vec<String> },
    RegexPattern { field_path: String, pattern: String },
}
...
impl ConfigMigrator {
    ...
    pub fn migrate_content(&self, content: &str, config: MigrationConfig) -> Result<(String, MigrationResult)> {
        // Parse YAML content
    ...
    pub fn migrate_value(&self, value: Value, config: MigrationConfig) -> Result<(Value, MigrationResult)> {
        ...
        if detected_format == config.target_format && !config.strict_mode {
            return Ok((value, MigrationResult {
                success: true,
        ...
        self.handle_legacy_list_format(&mut migrated_value, &mut applied_migrations, &mut warnings)?;
        ...
        match (&detected_format, &config.target_format) {
            (ConfigFormat::PreCommitPython { .. }, ConfigFormat::SnpV1 { .. }) => {
                ...
                self.apply_precommit_to_snp_migrations(&mut migrated_value, &mut applied_migrations, &mut warnings)?;
            },
            _ => {
        ...
        Ok((migrated_value, MigrationResult {
    ...
    fn apply_precommit_to_snp_migrations(
        ...
        if let Value::Mapping(map) = value {
                    ...
                    if let Value::Mapping(repo_map) = repo {
                        ...
                        if let Some(sha_value) = repo_map.remove(Value::String("sha".to_string())) {
                            ...
                            applied_migrations.push(AppliedMigration {
                                ...
                                new_value: Some(repo_map.get(Value::String("rev".to_string())).unwrap().clone()),
                        ...
                        if let Some(Value::Sequence(hooks)) = repo_map.get_mut(Value::String("hooks".to_string())) {
                            for hook in hooks {
                                if let Value::Mapping(hook_map) = hook {
                                    if let Some(Value::String(language)) = hook_map.get_mut(Value::String("language".to_string())) {
                                        if language == "python_venv" {
                                            let old_value = Value::String(language.clone());
                                            *language = "python".to_string();
                                            applied_migrations.push(AppliedMigration {
                                                migration_id: "language_normalization".to_string(),
                                                description: "Normalized 'python_venv' to 'python'".to_string(),
                                                field_path: "repos[*].hooks[*].language".to_string(),
                                                old_value: Some(old_value),
                                                new_value: Some(Value::String("python".to_string())),
                                                transformation_type: TransformationType::ValueTransform {
                                                    transformer: "language_normalizer".to_string(),
                                                },
                                            });
                                        }
                                    }
                                    ...
                                    if let Some(Value::Sequence(stages)) = hook_map.get_mut(Value::String("stages".to_string())) {
                                        for stage in stages {
            ...
            if let Some(Value::Sequence(default_stages)) = map.get_mut(Value::String("default_stages".to_string())) {
                for stage in default_stages {
                    if let Value::String(stage_name) = stage {
                        ...
                        applied_migrations.push(AppliedMigration {
                            ...
                            description: format!("Updated default stage name '{old_name}' to '{stage_name}'"),
    ...
    fn handle_legacy_list_format(
        ...
        if let Value::Sequence(repos_list) = value.clone() {
            ...
            new_map.insert(Value::String("repos".to_string()), Value::Sequence(repos_list.clone()));
            ...
            warnings.push(MigrationWarning {
                ...
                suggestion: Some("Consider updating your configuration to use the modern format".to_string()),
            });
    ...
    pub fn detect_format(&self, content: &str) -> Result<ConfigFormat> {
        ...
        if let Value::Mapping(map) = &yaml_value {
            ...
            if map.contains_key(Value::String("repos".to_string())) {
                ...
                if let Some(Value::String(version)) = map.get(Value::String("minimum_pre_commit_version".to_string())) {
                    return Ok(ConfigFormat::PreCommitPython { version: version.clone() });
                }
                // Default to recent pre-commit version
                return Ok(ConfigFormat::PreCommitPython { version: "3.0.0".to_string() });
            }
        } else if let Value::Sequence(_) = &yaml_value {
            // Legacy pre-commit format (top-level list)
            return Ok(ConfigFormat::PreCommitPython { version: "2.0.0".to_string() });
        }
        
        // Default to SNP v1 if no pre-commit markers found
        Ok(ConfigFormat::SnpV1 { version: "1.0.0".to_string() })
    }
    ...
    // Batch operations
    pub fn migrate_directory(&self, dir: &Path, config: MigrationConfig) -> Result<Vec<MigrationResult>> {
    ...
    pub fn migrate_repository(&self, repo_root: &Path, config: MigrationConfig) -> Result<RepositoryMigrationResult> {
...
impl Default for MigrationConfig {
    fn default() -> Self {
        Self {
            source_format: ConfigFormat::PreCommitPython { version: "latest".to_string() },
            target_format: ConfigFormat::SnpV1 { version: "1.0.0".to_string() },
...
"#;
        ...
        assert_eq!(result, ConfigFormat::PreCommitPython { version: "3.0.0".to_string() });
...
"#;
        ...
        assert_eq!(result, ConfigFormat::PreCommitPython { version: "2.0.0".to_string() });
        
        // Test compatibility checking
        let from_format = ConfigFormat::PreCommitPython { version: "3.0.0".to_string() };
        let to_format = ConfigFormat::SnpV1 { version: "1.0.0".to_string() };
    ...
    fn test_field_migration() {
        ...
        let yaml_with_sha = serde_yaml::from_str(r#"
repos:
- repo: https://github.com/test/repo
  sha: v1.2.3
  hooks:
...
"#).unwrap();
        ...
        let has_sha_to_rev_migration = result.applied_migrations.iter()
            .any(|m| m.migration_id == "sha_to_rev");
        assert!(has_sha_to_rev_migration, "Should have applied sha->rev migration");
    ...
    fn test_structure_migration() {
        ...
        let legacy_list_config = serde_yaml::from_str(r#"
- repo: https://github.com/test/repo
  rev: v1.0.0
  hooks:
  - id: test
"#).unwrap();
        ...
        let has_legacy_migration = result.applied_migrations.iter()
            .any(|m| m.migration_id == "legacy_list_to_repos");
        assert!(has_legacy_migration, "Should have applied legacy list migration");
    ...
    fn test_version_compatibility() {
        ...
        let old_version = ConfigFormat::PreCommitPython { version: "2.0.0".to_string() };
        let new_version = ConfigFormat::SnpV1 { version: "1.0.0".to_string() };
        ...
        let very_old_format = ConfigFormat::PreCommitPython { version: "1.0.0".to_string() };
        let latest_format = ConfigFormat::SnpV2 { version: "2.0.0".to_string() };
        
        let complex_plan = migrator.analyze_migration_needs(very_old_format, latest_format);
        assert_eq!(complex_plan.estimated_complexity, MigrationComplexity::Moderate);
        
        // Test same version (no migration needed)
        let same_format1 = ConfigFormat::SnpV1 { version: "1.0.0".to_string() };
        let same_format2 = ConfigFormat::SnpV1 { version: "1.0.0".to_string() };
        
        let no_migration_plan = migrator.analyze_migration_needs(same_format1, same_format2);
        assert_eq!(no_migration_plan.estimated_complexity, MigrationComplexity::Trivial);
    ...
    fn test_migration_validation() {
        ...
        let test_config = serde_yaml::from_str(r#"
repos:
- repo: https://github.com/test/repo
  rev: v1.0.0
  hooks:
...
"#).unwrap();
    ...
    fn test_batch_migration() {
        ...
        let results = migrator.migrate_directory(&temp_dir, config.clone()).unwrap();
...
"#;
        ...
        let (migrated_content, result) = migrator.migrate_content(test_content, MigrationConfig::default()).unwrap();
        assert!(result.success);
    ...
    fn test_migration_config_creation() {
        ...
        let custom_config = MigrationConfig {
            source_format: ConfigFormat::PreCommitPython { version: "2.0.0".to_string() },
            target_format: ConfigFormat::SnpV2 { version: "2.0.0".to_string() },
    ...
    fn test_format_equality() {
        let format1 = ConfigFormat::PreCommitPython { version: "3.0.0".to_string() };
        let format2 = ConfigFormat::PreCommitPython { version: "3.0.0".to_string() };
        let format3 = ConfigFormat::SnpV1 { version: "1.0.0".to_string() };
    ...
    fn test_migration_rule_creation() {
        let rule = MigrationRule {
            ...
            from_format: ConfigFormat::PreCommitPython { version: "3.0.0".to_string() },
            to_format: ConfigFormat::SnpV1 { version: "1.0.0".to_string() },
