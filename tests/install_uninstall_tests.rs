...
fn create_snp_config(repo_path: &PathBuf) {
...
async fn test_install_hooks_clean_repository() {
    ...
    let git_repo = GitRepository::discover().unwrap();
...
async fn test_hook_backup_creation() {
    ...
    let result = backup_manager.backup_hook(
        snp::install::HookType::PreCommit,
        &hook_path
    ).await;
...
async fn test_hook_backup_restoration() {
    ...
    let result = backup_manager.restore_hook(snp::install::HookType::PreCommit).await;
    assert!(result.is_ok());
...
async fn test_backup_cleanup() {
    ...
    for i in 1..5 {
        ...
        let old_time = std::time::SystemTime::now() - std::time::Duration::from_secs(60 * 60 * 24 * 35); // 35 days ago
    ...
    assert_eq!(cleanup_result.cleaned_files, 4);
    assert_eq!(cleanup_result.bytes_freed, 32); // 4 files * 8 bytes each
}
...
async fn test_hook_permissions_unix() {
    ...
    assert!(permissions.mode() & 0o100 != 0, "Owner should have execute permission");
    // Check that it's readable
    assert!(permissions.mode() & 0o400 != 0, "Owner should have read permission");
}
...
async fn test_hook_script_generation() {
    ...
    let result = template_generator.generate_hook_script(
        snp::install::HookType::PreCommit,
        &hook_config
    );
    assert!(result.is_ok());
...
async fn test_install_not_git_repository() {
    ...
    let result = GitRepository::discover();
    assert!(result.is_err());
...
async fn test_cli_install_command() {
    ...
    assert_eq!(result, 0); // Success exit code
...
#[tokio::test]
async fn test_cli_uninstall_command() {
    let (_temp_dir, repo_path) = create_test_git_repo();
    create_snp_config(&repo_path);
    ...
    // First install hooks
    let install_args = vec!["snp", "install"];
    let install_cli = Cli::try_parse_from(install_args).unwrap();
    let install_result = install_cli.run().unwrap();
    ...
    let uninstall_result = uninstall_cli.run().unwrap();
    assert_eq!(uninstall_result, 0);
...
async fn test_installation_performance() {
    ...
    let install_config = snp::install::InstallConfig {
        hook_types: vec!["pre-commit".to_string(), "pre-push".to_string(), "commit-msg".to_string()],
        overwrite_existing: false,
        allow_missing_config: false,
        backup_existing: true,
        hook_args: vec![],
    ...
    assert!(duration.as_millis() < 100, "Installation took too long: {}ms", duration.as_millis());
}