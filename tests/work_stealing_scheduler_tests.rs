...
use snp::{
    ...
    core::{Hook, Stage, ExecutionContext},
    lock_free_scheduler::{
        WorkStealingScheduler, Task, TaskPayload, SchedulerConfig
    },
...
fn create_heterogeneous_tasks(count: usize) -> Vec<Task> {
    ...
    for i in 0..count {
        ...
        let task = Task {
            ...
            dependencies: vec![],
            payload: TaskPayload::HookExecution { hook: Box::new(hook), files, context },
...
mod work_stealing_algorithm_tests {
    ...
    async fn test_work_stealing_load_balancing() {
        ...
        let max_tasks = statistics.iter().map(|s| s.tasks_executed.load(std::sync::atomic::Ordering::Relaxed)).max().unwrap();
        let min_tasks = statistics.iter().map(|s| s.tasks_executed.load(std::sync::atomic::Ordering::Relaxed)).min().unwrap();
        
        // Allow some imbalance but should be reasonable
        assert!(max_tasks - min_tasks <= 30, "Load imbalance too high: max={max_tasks}, min={min_tasks}");
        assert!(metrics.load_balance_coefficient < 0.3, "Load balance coefficient too high: {}", metrics.load_balance_coefficient);
    ...
    async fn test_steal_success_rate() {
        ...
        assert!(metrics.steal_success_rate > 0.3, "Steal success rate too low: {}", metrics.steal_success_rate);
...
mod task_priority_tests {
    ...
    async fn test_task_timeout_handling() {
        ...
        let task = Task {
            ...
            priority: TaskPriority::Normal,
            estimated_duration: Some(Duration::from_millis(200)), // Longer than default timeout
            dependencies: vec![],
            payload: TaskPayload::HookExecution { hook: Box::new(hook), files, context },
            created_at: Instant::now(),
        };
...
mod dependency_resolution_tests {
    ...
    async fn test_simple_task_dependencies() {
        ...
        let receivers = resolver.resolve_and_submit(tasks, &scheduler).await.unwrap();
...
mod performance_tests {
    ...
    async fn test_throughput_vs_single_threaded() {
        ...
        for task in &tasks {
            ...
            tokio::time::sleep(task.estimated_duration.unwrap_or(Duration::from_millis(10))).await;
        }
        ...
        let speedup = single_threaded_duration.as_millis() as f64 / work_stealing_duration.as_millis() as f64;
        ...
        assert!(speedup > 2.0, "Work-stealing speedup too low: {speedup:.2}x");
    ...
    async fn test_worker_utilization() {
        ...
        // Should have good worker utilization
        let avg_utilization = metrics.worker_utilization.iter().sum::<f32>() / metrics.worker_utilization.len() as f32;
        assert!(avg_utilization > 0.7, "Worker utilization too low: {avg_utilization:.2}");
    ...
    async fn test_integration_with_hook_execution() {
        ...
        for hook in hooks.into_iter() {
            let task = Task {
                ...
                priority: if hook.id == "mypy" { TaskPriority::High } else { TaskPriority::Normal },
