...
use snp::storage::{Store, RepositoryInfo, EnvironmentInfo, ConfigInfo};
...
impl TestEnvironment {
    ...
    pub fn create_mock_repository(&self, url: &str, revision: &str, age_days: u32) -> Result<PathBuf> {
        let deps = vec![];
    ...
    pub fn create_mock_environment(&self, language: &str, deps: &[String], age_days: u32) -> Result<PathBuf> {
    ...
    fn set_repository_age(&self, url: &str, revision: &str, deps: &[String], time: SystemTime) -> Result<()> {
...
async fn test_cache_analysis_discovery() {
    ...
    let _repo1 = env.create_mock_repository("https://github.com/test/repo1", "v1.0", 10).unwrap();
    let _repo2 = env.create_mock_repository("https://github.com/test/repo2", "v2.0", 5).unwrap();
    let _env1 = env.create_mock_environment("python", &["requests==2.28.0".to_string()], 15).unwrap();
    let _env2 = env.create_mock_environment("rust", &["serde==1.0".to_string()], 3).unwrap();
    ...
    assert!(repos.iter().any(|r| r.url == "https://github.com/test/repo1"));
    assert!(repos.iter().any(|r| r.url == "https://github.com/test/repo2"));
...
async fn test_unused_repository_detection() {
    ...
    let _recent_repo = env.create_mock_repository("https://github.com/test/recent", "v1.0", 1).unwrap();
    let _old_repo = env.create_mock_repository("https://github.com/test/old", "v1.0", 35).unwrap();
...
async fn test_orphaned_environment_detection() {
    ...
    // Create environments
    let env_path = env.create_mock_environment("python", &["requests==2.28.0".to_string()], 1).unwrap();
...
async fn test_space_usage_calculation() {
    ...
    let _repo = env.create_mock_repository("https://github.com/test/repo", "v1.0", 1).unwrap();
    let _env = env.create_mock_environment("python", &["requests".to_string()], 1).unwrap();
...
async fn test_repository_cache_cleanup() {
    ...
    // Create repositories
    let _recent_repo = env.create_mock_repository("https://github.com/test/recent", "v1.0", 1).unwrap();
    let old_repo_path = env.create_mock_repository("https://github.com/test/old", "v1.0", 35).unwrap();
    ...
    let cleaned_count = env.store.cleanup_environments(Duration::from_secs(30 * 24 * 60 * 60)).unwrap(); 
...
async fn test_environment_cleanup() {
    ...
    let _recent_env = env.create_mock_environment("python", &["requests".to_string()], 1).unwrap();
    let old_env_path = env.create_mock_environment("rust", &["serde".to_string()], 35).unwrap();
...
async fn test_selective_cleanup() {
    ...
    let _repo = env.create_mock_repository("https://github.com/test/repo", "v1.0", 35).unwrap();
    let _env = env.create_mock_environment("python", &["requests".to_string()], 35).unwrap();
...
async fn test_usage_tracking() {
    ...
    let repo_path = env.create_mock_repository("https://github.com/test/repo", "v1.0", 1).unwrap();
    let env_path = env.create_mock_environment("python", &["requests".to_string()], 1).unwrap();
...
async fn test_database_consistency_after_cleanup() {
    ...
    for repo in repos {
        if !repo.path.exists() {
            panic!("Repository path does not exist but is still in database: {:?}", repo.path);
        }
    }
    
    for env in envs {
        if !env.path.exists() {
            panic!("Environment path does not exist but is still in database: {:?}", env.path);
        }
    }
}
