...
use snp::language::golang::{
    GoLanguagePlugin, GoToolchainInfo, GoModule, ModuleMode, GoDependency, 
    ReplaceDirective, GoEnvironment, GoInstallationSource, GoTool,
    GoError, GoBuildConfig, GoCacheConfig, OptimizationLevel
};
use snp::language::traits::Language;
use snp::language::environment::{EnvironmentConfig, LanguageEnvironment, IsolationLevel, CacheStrategy};
...
async fn test_go_toolchain_detection() {
    ...
    match plugin.detect_go_installations().await {
        Ok(installations) => {
            assert!(!installations.is_empty(), "Should find at least one Go installation");
            
            for installation in &installations {
                assert!(installation.go_executable.exists(), "Go executable should exist");
                assert!(!installation.version.to_string().is_empty(), "Version should be detected");
...
async fn test_go_installation_path_and_goroot_detection() {
    ...
    if let Ok(installations) = plugin.detect_go_installations().await {
        for installation in installations {
            ...
            assert!(installation.goroot.join("bin").exists(), "GOROOT/bin should exist");
...
async fn test_cross_platform_go_installation_handling() {
    ...
    let test_installations = vec![
        (PathBuf::from("/usr/local/go/bin/go"), PathBuf::from("/usr/local/go")),
        (PathBuf::from("/opt/go/bin/go"), PathBuf::from("/opt/go")),
        (PathBuf::from("C:\\Program Files\\Go\\bin\\go.exe"), PathBuf::from("C:\\Program Files\\Go")),
    ];
...
"#;
    ...
    fs::write(module_path.join("go.mod"), go_mod_content).await.unwrap();
    
    // Create go.sum file
    let go_sum_content = r#"github.com/stretchr/testify v1.8.4 h1:CcVxjf3Q8VM5dzMtD9rkQKUSV4I4C3hg0CvAmf8kNaM=
github.com/stretchr/testify v1.8.4/go.mod h1:sz/lmYIOXD/1dqDmKjjqLyZ2RngseejIcXlSw2iwfAo=
"#;
    fs::write(module_path.join("go.sum"), go_sum_content).await.unwrap();
...
"#;
    fs::write(workspace_path.join("go.work"), go_work_content).await.unwrap();
    
    // Create module directories
    fs::create_dir_all(workspace_path.join("module1")).await.unwrap();
    fs::create_dir_all(workspace_path.join("module2")).await.unwrap();
    
    fs::write(workspace_path.join("module1/go.mod"), "module example.com/module1\ngo 1.21").await.unwrap();
    fs::write(workspace_path.join("module2/go.mod"), "module example.com/module2\ngo 1.21").await.unwrap();
...
async fn test_module_proxy_and_goproxy_configuration() {
    ...
    assert_eq!(plugin.config.goproxy_url, Some("https://proxy.golang.org".to_string()));
...
"#;
    fs::write(module_path.join("go.mod"), go_mod_with_deps).await.unwrap();
    
    let module_with_deps = plugin.detect_go_module(module_path).await.unwrap();
    assert_eq!(module_with_deps.dependencies.len(), 1);
    assert_eq!(module_with_deps.dependencies[0].module_path, "github.com/example/lib");
...
"#;
    ...
    let local_replace = module.replace_directives.iter()
    ...
    let version_replace = module.replace_directives.iter()
...
"#;
    fs::write(module_path.join("main.go"), main_go).await.unwrap();
    ...
    let hook = Hook::new("test-go", "go run main.go", "golang")
        .with_name("Test Go Hook");
...
"#;
    fs::write(module_path.join("main.go"), formatter_go).await.unwrap();
    ...
    let hook = Hook::new("formatter", "go run main.go", "golang")
        .with_name("Go Formatter");
    
    let test_files = vec![
        module_path.join("file1.go"),
        module_path.join("file2.go"),
    ];
...
"#;
    fs::write(module_path.join("main.go"), tool_go).await.unwrap();
    ...
    let result = plugin.install_go_dependencies(&fake_toolchain, &fake_module, &deps).await;
...
async fn test_build_environment_setup() {
    ...
    let go_env = plugin.setup_go_environment(&fake_toolchain, &fake_module, &[]).await.unwrap();
    ...
    assert_eq!(go_env.environment_vars.get("GOPROXY"), Some(&"https://proxy.golang.org".to_string()));
    assert_eq!(go_env.environment_vars.get("GOSUMDB"), Some(&"sum.golang.org".to_string()));
    assert_eq!(go_env.environment_vars.get("GO111MODULE"), Some(&"on".to_string()));
    assert_eq!(go_env.environment_vars.get("CGO_ENABLED"), Some(&"0".to_string()));
}
...
async fn test_cgo_and_build_constraints_handling() {
    ...
    assert_eq!(plugin.config.build_config.build_tags, vec!["integration", "test"]);
...
async fn test_parallel_build_configuration() {
    ...
    assert_eq!(plugin.config.build_config.optimization_level, OptimizationLevel::Speed);
...
async fn test_vendor_directory_support() {
    ...
    fs::create_dir_all(module_path.join("vendor")).await.unwrap();
    fs::write(
