...
use snp::{
    ...
    concurrency::{TaskConfig, ConcurrencyExecutor, ResourceLimits},
};
...
mod lock_free_cache_tests {
    ...
    fn test_cache_bulk_operations() {
        ...
        let entries = (0..50).map(|i| (format!("key_{}", i), format!("value_{}", i))).collect::<Vec<_>>();
...
mod integration_tests {
    ...
    fn test_concurrency_executor_lock_free_integration() {
        let executor = ConcurrencyExecutor::with_lock_free_scheduler(
            4,
    ...
    async fn test_performance_comparison_setup() {
        ...
        let lock_free_executor = ConcurrencyExecutor::with_lock_free_scheduler(
            4,
            ResourceLimits::default()
        );
